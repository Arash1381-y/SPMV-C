...
int main() {
  ...
  clock_t start, end;
  double elapsed_time_baseline, elapsed_time_st, elapsed_time_mt,
      elapsed_time_vectorize, elapsed_time_csr;
  ...
  // create sparse matrix and measure the time taken to convert the matrix to
  // sparse CSR format
  start = clock();
  ...
  // Measure the time taken by the vectorized sparse CSR multiplication with 1
  // thread
  start = clock();
  ...
  printf("REPORT OF SPARSE MATRIX MULTIPLICATION WITH %zu ROWS AND %zu COLUMNS "
         "AND SPARSITY OF %f\n",
         row, col, sparsity);

  printf("_____________________________________________________________________"
         "___\n");
  printf("|    Method     | Time taken (s) | Speedup  | Efficiency | "
         "Correctness |\n");
  printf("|---------------|----------------|----------|------------|-----------"
         "--|\n");
  printf(
  ...
  printf("|---------------|----------------|----------|------------|-----------"
         "--|\n");
  printf(
      "| Single thread | %.3f          |  %.3f   |  %.3f     | %s        |\n",
      elapsed_time_st, elapsed_time_baseline / elapsed_time_st,
      (elapsed_time_baseline / elapsed_time_st) / 1,
      is_equal(baseline_result, st_result) ? "true" : "false");
  printf("|---------------|----------------|----------|------------|-----------"
         "--|\n");
  printf(
      "|  Multi thread | %.3f          |  %.3f   |  %.3f     | %s        |\n",
      elapsed_time_mt, elapsed_time_baseline / elapsed_time_mt,
      (elapsed_time_baseline / elapsed_time_mt) / 4,
      is_equal(baseline_result, mt_result) ? "true" : "false");
  printf("|---------------|----------------|----------|------------|-----------"
         "--|\n");
  printf(
      "|   Vectorized  | %.3f          |  %.3f   |  %.3f     | %s        |\n",
      elapsed_time_vectorize, elapsed_time_baseline / elapsed_time_vectorize,
  ...
  printf(
      "|       CSR     |  %.3f         | %.3f    | %.3f      | %s        |\n",
      elapsed_time_csr, elapsed_time_baseline / elapsed_time_csr,
      (elapsed_time_baseline / elapsed_time_csr) / 1,
      is_equal(baseline_result, st_result) ? "true" : "false");
  printf("|---------------|----------------|----------|------------|-----------"
         "--|\n");
