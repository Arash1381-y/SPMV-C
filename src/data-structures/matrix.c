...
void init_matrix(matrix *m, size_t row, size_t col, bool random_init,
                 double sparsity) {
  m->n_rows = row;
  m->n_cols = col;
...
void init_sparse_csr(matrix *m, sparse_CSR *s) {
  size_t n_rows, n_cols;
  n_rows = s->n_rows = m->n_rows;
  n_cols = s->n_cols = m->n_cols;

  size_t non_zeros = n_rows * n_cols -
                     count_initial_zeros(m->flatt_val, s->n_cols * s->n_rows);
  s->n_nz = non_zeros;
...
void free_matrix(matrix *m) { free(m->flatt_val); }
